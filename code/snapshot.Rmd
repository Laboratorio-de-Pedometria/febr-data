...
```{r}
datasets <- febr::dataset(dataset = 'all')
```
...
datasets <- t(datasets) %>% data.frame()
colnames(datasets) <- c(dts_rows, "area_conhecimento", "autor_nome", "organizacao_nome")
col_order <- 
...
grid()
barplot(
  tmp, col = "firebrick1", border = "firebrick1",
