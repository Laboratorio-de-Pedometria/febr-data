...
doze variáveis do solo disponíveis no ___febr___. São elas:
...
* Tabela _camada_:
  ...
  - `ph`: pH em água da fração terra fina, se unidade de medida;
...
de coordenadas espaciais utilizado é o SIRGAS 2000, com coordenadas espacials expressas em graus decimais.

Os passos de limpeza dos dados descarregados são descritos abaixo, enquanto que os os passos de padronização e
...
1. Fusão das colunas com a classificação taxonômica do solo nas diferentes versões de um mesmo sistema de 
   ...
   Classificação do Solo, `sibcs`, classificações anteriores a 1999 são ignoradas, pois a nomenclatura 
   utilizada é muito diferente da nomenclatura atual. Nesse caso, observações apenas com a classificação
...
O ___febr___ dispõe de um total de 14664 observações do solo. Destas, 11580 possuem coordenadas espaciais, ou
seja, 11580 / 14664 * 100 = 79%. A precisão mediana das coordenadas espaciais é de 100 m. A fonte de boa parte
das coordenadas é desconhecida, sendo o GPS a fonte mais comum. Boa parte das coordenadas foi estimada usando
serviços de mapas online (WEB), ou então foram estimadas usando, por exemplo, mapas base (MAPA), sendo 
posteriormente ajustadas ou corrigidas usando serviços online como o Google Maps.

```{r}
br <- sf::read_sf("../data/br.shp")
tmp <- 
...
plot(
  ...
  sub = Sys.Date(), main = "Distribuição espacial das observações", 
  col = 0, xlab = "Longitude", ylab = "Longitude", reset = FALSE)
observacao %>% 
...
aparecem. O estado de Rondônia concentra o maior número de observações, especificamente, 3031 observações. 
Enquanto isso, o menos número de observações parece no estado do Tocantis -- apenas 71 observações. Devido ao 
seu relativamente grande território, o estado do Tocantis também possui a menor densidade de observação: 0,26 
observações a cada 1000 km². A maior densidade de observação é no Distrito Federal, com 27 observações a cada
1000 km². Em geral, os estados das regiões Centro-Oeste, Norte e Nordeste são aqueles com as menores densidades
de observação, refletindo o histórico de ocupação do território brasileiro e os investimentos feitos no ensino
e pesquisa via universidades e centros de pesquisa agronômica.
...
tmp %>% 
  sort() %>% 
  barplot(
    col = "firebrick1", border = "firebrick1", horiz = TRUE, ann = FALSE,
    ylab = "Unidade da federação", xlab = "Número de observações")
grid()
dens %>% 
  barplot(
    col = "firebrick1", border = "firebrick1", horiz = TRUE, ann = FALSE,
    ylab = "Unidade da federação", xlab = "Densidade de observações (por 1000 km²)")
title(main = "Distribuição das observações pelas unidades da federação", sub = Sys.Date(), outer = TRUE)
grid()
```

Das observações sem coordenadas espaciais (14664 - 11580 = 3084), a maioria se encontra nos estados da Bahia
(434), Minas Gerais (383), Paraná (348) e Pará (289). Três conjuntos de dados concentram parte considerável
dessas observações -- ctb0657, ctb0831 e ctb0775 --, cada um com mais de 100 observações sem coordenadas
espaciais. Outros 14 conjuntos de dados possuem entre 50 e 100 observações sem coordenadas espaciais.
...
observacao %>% 
  ...
  sort() %>% 
  barplot(
    col = "firebrick1", border = "firebrick1", horiz = TRUE,
    ylab = "Unidade da federação", xlab = "Número de observações")
grid()
observacao %>% 
  dplyr::filter(is.na(coord_x)) %>% 
...
grid()
title(main = "Distribuição das observações sem coordenadas espaciais", sub = Sys.Date(), outer = TRUE)
```

```{r}
observacao %>% 
  mutate(observacao_data = as.Date(observacao_data) %>% format("%Y")) %>% 
  summarise(
    Total = n(),
...
Apenas cerca de 9693 / 14664 * 100 = 66% das observações do solo possuem registro da data de observação. 
Destas, metadade -- 51% -- foram obtidas entre 1990 e 2010. Isso se deve a um pico no ano de 1997, ano em que
foram obtidas as observações do solo do maior conjunto de dados disponível no ___febr___. Trata-se do conjunto
de dados do Zoneamento Agroecológico do Estado de Rondônia, que possui mais de 2000 observações do solo. Das
observações sem data de observação, acredita-se que 

Acredita-se que esse número seja bastante
maior, pois se sabe que a maioria dos conjuntos de dados hoje disponíveis no ___febr___ foram produzidos nesse
período, principalmente na década de 1970.
...
```{r}
tmp <- observacao[!is.na(tmp$coord_x), c("coord_x", "coord_y", "observacao_data")]
idx <- duplicated(tmp)
sum(idx)
```
...
grid()
barplot(
  tmp, col = "firebrick1", border = "firebrick1", add = TRUE, xlab = "Ano", ylab = "Número de observações",
  main = "Distribuição temporal das observações", sub = glue::glue("Versão {Sys.Date()}"))
# dev.off()
```

A maioria das observações sem data de observação pertencem a conjuntos de dados que abrangem os estados do 
Amazonas, Rio Grande do Sul, Bahia, Pará, Minas Gerais e Paraná. Dentre estes, três conjuntos de dados se 
destacam pelo grande número de observações sem coordenadas. São eles: ctb0770, no Rio Grande do Sul, ctb0657, 
na Bahia, e ctb831, no Paraná.
...
observacao %>% 
  ...
  barplot(
    ...
    ylab = "Conjunto de dados", xlab = "Número de observações")
grid()
title(main = "Distribuição das observações sem data", sub = Sys.Date(), outer = TRUE)
```
