...
### Correção de valores I

Alguns dos conjuntos de dados disponíveis no ___febr___ ainda não passaram pelo processo de validação. Isso 
significa que os dados descarregados podem conter inconsistências, sejam elas oriundas dos trabalhos originais
ou de equívocos durante a entrada dos dados nas planilhas eletrônicas. Assim, os dados passam pelos seguintes
...
Após a correção dos dados das variáveis prioritárias, faz-se a fusão das colunas dos dados de cada variável
determinadas usando métodos parcialmente distintos. Isso é feito usando usando `dplyr::coalesce()`.
...
### Correção de valores II

A segunda etapa do processamento destinada à correção dos dados das variáveis prioritárias consiste dos 
seguintes passos:

1. Na falta de uma das três frações granulométricas finas -- `argila`, `silte` e `areia` --, calcula-se seu
   valor como sendo a diferença entre 1000 g/kg e a soma das duas outras frações granulométricas. Isso é mais
   comum no caso do silte. Na falta dos dados do conteúdo de areia total, primeiro se usa (quando disponível) 
   os dados do conteúdo de areia grossa + areia fina.
3. 
...
camada %>% 
  dplyr::mutate(
    ...
    terrafina = ifelse(!terrafina_in & !cascalho_in & !calhau_in, 1000, terrafina),
  ...
  filter(dtp != 1000) %>% 
  select(dataset_id, observacao_id, camada_nome, areia, argila, silte, dtp)
